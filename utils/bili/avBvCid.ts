...
export const bv2av = (bvid: `BV1${string}`) => {
...
// return a list of cid
export async function bv2Cid(bvid: string) {
  const res = await fetch(
    `https://api.bilibili.com/x/player/pagelist?bvid=${bvid}&jsonp=jsonp`
  );
  const json = await res.json();
  return json.data as Array<{ cid: number; part: string; duration: number }>;
}

export async function av2Cid(aid: number) {
  return bv2Cid(av2bv(aid));
}
